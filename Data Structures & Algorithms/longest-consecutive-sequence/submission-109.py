class Solution:
    ...
    # The if statement before the inner loop is what ensures the inner loop only runs at the
    # beginning of sequeuces. The length counting would work without it, but it would then run
    # multiple times for each iteration. Instead of o(n + n) it would be O(n*n). 
    # IF YOU CAN GATEKEEP INNER LOOPS, YOU CAN MAKE NESTED LOOPS EFFICIENT
